  ...
  try {
    ...
    return {
      ...
      review: {
        ...
        hold_reason_code: 'sentinel_review_error',
        hold_risk_category: 'sentinel_runtime',
        hold_confidence: 1,
        suggested_next_action: 'Route this node output to review before continuing the workflow.',
        explainability: {
          verdict: 'HOLD',
          riskScore: 1,
          policyTrigger: 'sentinel_review_error',
          riskCategory: 'sentinel_runtime',
          confidence: 1,
        },
      },
      result,
...
function buildHoldResult({ review, result, schemaValidation, repairAttempted, repairAttempts = 0, sentinelFinalVerdict, sentinelProviderFallback }) {
  return {
    ...
    review: {
      ...
      hold_reason_code: review?.hold_reason_code ?? 'sentinel_hold_after_repair',
      hold_risk_category: review?.hold_risk_category ?? 'quality_gate',
      hold_confidence: review?.hold_confidence ?? review?.explainability?.confidence ?? null,
    },
    result,
    schemaValidation,
    repairAttempted,
